...
export default function TablaInformacionMecanico() {
    ...
    useEffect(() => {
        // Simulación de inicio de sesión y obtención de ID y rol del usuario
        const loggedInUserId = localStorage.getItem('userId');
        const loggedInUserRole = localStorage.getItem('userRole');
        setUserId(loggedInUserId);
        setUserRole(loggedInUserRole);
        fetchUsersByUserId(loggedInUserId); // Llama a la función para obtener los datos según el ID del usuario
    }, []);

    const fetchUsersByUserId = async (userId) => {
        try {
            const response = await axios.get(`https://localhost:3000/api/v1/registro-de-trabajos?userId=${userId}`);
    ...
    useEffect(() => {
        ...
        if (mechanicId) {
            fetchUsersByMechanicId(mechanicId);
        }
    }, []);

    const fetchUsersByMechanicId = async (mechanicId) => {
        try {
            const response = await axios.get(`https://localhost:3000/api/v1/registro-de-trabajos?mecanicoId=${mechanicId}`);
            setUsers(response.data);
        } catch (error) {
            console.error("Error fetching registro:", error);
        }
